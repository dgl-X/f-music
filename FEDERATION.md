...
В v1 входят pairing, синхронизация метаданных, объединённый поиск, прокси-поток,
удалённые ссылки в очередях/плейлистах, отзыв доверия, аудит и локальный импорт
понравившегося оригинала. Публичного каталога узлов, глобальных аккаунтов и
relay в v1 нет.
...
Текущая реализация поддерживает `none`, `all`, выбранные `albums` и именованные
`collections`. Для каждого peer можно наследовать общую политику ноды либо
назначить собственную область публикации.
...
Потоковые данные имеют отдельный namespace origin/track и ограниченные временные
диапазоны. Лайк удалённого трека ставит фоновое задание: worker докачивает
оригинал через Range, проверяет размер и SHA-256, переносит обложку при её
отсутствии внутри файла и создаёт обычный независимый локальный трек. После
revoke новые чтения блокируются; уже завершённый локальный импорт от origin не
зависит. Таймауты и повторные ошибки ограничены, для peer действует circuit
breaker.
...
Relay для CGNAT, публичная discovery-сеть и более мелкая гранулярность экспорта.
