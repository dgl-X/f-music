...
Текущая стабильная версия сервера — **0.1.13**. Работают:
...
- создание обычных семейных аккаунтов администратором;
...
- пакетное оформление загруженной папки как альбома: отдельная карточка с ID, исполнителем, описанием и обложкой, номера дисков и дорожек, ручная перестановка;
...
- временный кэш веб-плеера в памяти вкладки: ближайшие два трека и текущий для повтора, до 128 МиБ суммарно и 64 МиБ на файл; на паузе предзагрузка останавливается, при выходе кэш очищается;
...
storage/albums/            обложки карточек альбомов
```
...
- каталог `storage/albums`;
- `storage/federation/identity.json`, если федерация включена;
...
- Самостоятельная регистрация по умолчанию выключена. Администратор может
  включить её в настройках; новые аккаунты всегда создаются без административных прав.
...
Сборка и установка стабильного Debian-пакета описана в
[`DEBIAN_PACKAGE.md`](DEBIAN_PACKAGE.md). Каждый push проверяется без сохранения
временного пакета, а amd64 DEB и `SHA256SUMS` публикуются только для тегов
`vX.Y.Z` в GitHub Releases.
