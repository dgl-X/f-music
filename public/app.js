...
async function showFederationSettings(){
  ...
  const loadPeers=async()=>{const root=form.querySelector('.peer-list');try{const data=await api('/api/v1/admin/federation/peers'),names={compatible:'Совместима',limited:'Ограниченная совместимость',upgrade_required:'Требуется обновление',revoked:'Отключена'};root.innerHTML=data.items.length?data.items.map(item=>`<div><span><strong>${escapeHtml(item.label||item.endpoint)}</strong><small>${names[item.status]||escapeHtml(item.status)} · удалённых треков: ${Number(item.remote_tracks||0)}</small><small>${item.sync_error?`Ошибка sync: ${escapeHtml(item.sync_error)}`:item.last_synced_at?`Синхронизация: ${new Date(item.last_synced_at).toLocaleString('ru-RU')}`:'Синхронизация ещё не выполнялась'}</small>${item.notify_error?`<small>Ошибка notify: ${escapeHtml(item.notify_error)}</small>`:''}<small>${escapeHtml(item.node_id)}</small></span>${item.status!=='revoked'?`<button type="button" class="secondary revoke-peer" data-id="${escapeHtml(item.node_id)}">Отключить</button>`:''}</div>`).join(''):'Подключённых нод пока нет.';root.querySelectorAll('.revoke-peer').forEach(button=>button.onclick=async()=>{if(!confirm('Отключить доверенную ноду?'))return;await api(`/api/v1/admin/federation/peers/${button.dataset.id}`,{method:'DELETE'});loadPeers();});}catch(problem){root.textContent=problem.message;}};
  ...
  form.onsubmit=async event=>{event.preventDefault();error.textContent='';try{state=await api('/api/v1/admin/federation',{method:'PUT',body:JSON.stringify({enabled:form.elements.enabled.checked,export_policy:form.elements.export_policy.value,selected_albums:selectedAlbums(),endpoints:endpoints()})});dialog.close();showWebSettings();}catch(problem){error.textContent=problem.message;}};
}
